...
func main() {
	...
	// Create the template
	tmpl := template.Must(template.New(importsGoFilename).Parse(importsGoTmpl))

	// Create the imports.go file
	f, err := os.Create(importsGoFilename)
	if err != nil {
		log.Fatalf("error creating file: %q: %+v", importsGoFilename, err)
	}
